...
switch ($Action){
    ...
    /****************************************************************************
	 * DADOS DAS CAMPANHAS SIMPLES PARA A ROTA MARKETING/CAMPAIGNS
	 */
    case 'graphics/campaigns/basic':
        $Contact = _get_data_full(
            "SELECT DISTINCT `contact_link` 
            FROM `".TB_MKT_CONTACTS."` 
            WHERE `contact_account_id` = {$_SESSION['account_id']} 
            AND `contact_link` is NOT NULL "
        );
        $Labels = [];
        $Data = [];
        foreach ($Contact as $key => $a) {
            $Channel = _get_data_full(
                "SELECT `camp_link`,`camp_channel` 
                FROM `".TB_MKT_CAMPAIGNS."` 
                WHERE `camp_account_id` = {$_SESSION['account_id']} 
                AND `camp_link` = '{$a['contact_link']}'");
            $Channel = ($Channel[0]['camp_channel'])? $Channel[0]['camp_channel'] : $a['contact_link'];

            array_push($Labels, $Channel);
            $Count = _get_data_full(
                "SELECT COUNT(`contact_id`) AS total 
                FROM `".TB_MKT_CONTACTS."` 
                WHERE `contact_account_id` = {$_SESSION['account_id']} 
                AND `contact_link` =:a","a={$a['contact_link']}"
            );           
