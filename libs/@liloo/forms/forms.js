 ...
 * @copyright Felipe Oliveira - 05.09.2022
 * @version 1.1.3
 */
...
const Fields = {
    ...
    Input: function (element, props, rand) {
        // let elTootltipo = (typeof props.tooltip == 'undefined') ? '' : `<i uk-icon="${elToolIcom}" id="${Rand}"></i>`
        // let elTootltipo = (typeof props.tooltip == 'undefined') ? '' : `<i uk-icon="${elToolIcom}" data-tooltip="${props.tooltip}" data-position="top left" data-variation="small" data-inverted=""></i>`   
    ...
    Textarea: function (element, props, rand) {
        let elToolIcom = (typeof props.toolicon == 'undefined') ? 'question' : props.toolicon
        let elTootltipo = (typeof props.tooltip == 'undefined') ? '' : `<i uk-icon="${elToolIcom}" id="tool-${rand}"></i>`
        let elLabel = !props.label ? '' : `<label class="uk-form-label">${props.label} ${elTootltipo}</label>`
        let elId = !props.id ? '' : `id="${props.id}"`
    ...
    Select: function (element, props, rand) {
        let elToolIcom = (typeof props.toolicon == 'undefined') ? 'question' : props.toolicon
        let elTootltipo = (typeof props.tooltip == 'undefined') ? '' : `<i uk-icon="${elToolIcom}" id="tool-${rand}"></i>`
        let elLabel = !props.label ? '' : `<label class="uk-form-label">${props.label} ${elTootltipo}</label>`
        let elId = !props.id ? '' : `id="${props.id}"`
 ...
 * @param {*} Obj 
 */
const lilooForms = function (Obj = {}) {
    let element = 'liloo-form'
    let objSelector = (typeof Obj.selector == 'undefined') ? element : Obj.selector
    let objAction = (typeof Obj.action == 'undefined') ? '' : Obj.action
    let objPath = (typeof Obj.path == 'undefined') ? '' : Obj.path
    let objValue = (typeof Obj.value == 'undefined') ? '' : Obj.value
    let objlabelSubmit = (typeof Obj.labelSubmit == 'undefined') ? 'Enviar' : Obj.labelSubmit
    let objButtonSubmit = (typeof Obj.buttonSubmit == 'undefined') ? true : false
    element = objSelector

    // Opção de criar o botão submit
    let viewSubmit = (objButtonSubmit) ? `
    <div class="uk-margin">
        <button type="submit" class="uk-button uk-button-primary uk-button-small">${objlabelSubmit}</button>
    </div>
    ` : ''

    // Monta o form com as confgurações passadas
    $(element).html(`
        <form>
            ...
            <input type="hidden" name="action" value="${objAction}">
            <input type="hidden" name="path" value="${objPath}">
    ...
    if (Obj.build) {
        ...
        Obj.build.forEach((val) => {
            ...
            if (val.el == 'html') { Tpl += val.html }
            if (val.el == 'after') { Tpl += '</div>' + val.html }
            if (val.el == 'input') { Tpl += Fields.Input(element + ' .liloo-form-fields', val, (Rand + incr)) }
            if (val.el == 'textarea') { Tpl += Fields.Textarea(element + ' .liloo-form-fields', val, (Rand + incr)) }
            if (val.el == 'select') { Tpl += Fields.Select(element + ' .liloo-form-fields', val, (Rand + incr)) }
            incr++
        })
        $(element + ' .liloo-form-fields').html(Tpl)
        $('.toggle').checkbox()
        lilooMask()
        incr = 1
        Obj.build.forEach((val) => {
    ...
    $(element + ' form').on('submit', function (e) {
        e.preventDefault()
        let dimmer = $(element + ' form .callback-dimmer')
        let form = new FormData(this)
        $(element + ' form .callback-message').hide()
