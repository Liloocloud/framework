<?php
/**
 * Page: Lista de orçamentos solicitados pelos usuários
...
$Budgets = new Paginator(		
	...
	AND `budget_favorites` NOT LIKE '%#{$Company['account_id']}#%'

	AND `budget_zipcode` LIKE '".substr($Company['company_zipcode'], 0, 2)."%'  
...
if($Budgets->Results()):
	...
	foreach ($Budgets->Results() as $Budget):
		...
		// endif;
		// Monta o checkbox se existir
		// if(isset($bud_details['add_problem']) && !empty($bud_details['add_problem'])){
		
		// 	$add_problem = "<p>{$ser_details['add_problem']['label']}<br>";
		// 	foreach ($bud_details['add_problem'] as $item_add_problem) {
		// 		$add_problem.= "<i uk-icon='check'></i>&nbsp; <b>{$item_add_problem}</b><br>";
		// 	}
		// 	$add_problem.= '</p>'; 
		// }
		...
		$Extra = [
			...
			'btn_rejected' => "<a href=\"#\" onclick=\"Flex.RequestServer('budget_salve_rejected','frontend/vamove/_php/Budgets','{$Budget['budget_id']},{$Company['account_id']}',true)\" class=\"uk-icon-button\" uk-icon=\"ban\"></a>",
		...
		$tpl.= _tpl_fill($PATH_DASH.'routes/orcamentos/listview.tpl', '', $Extra, false);
	...
	$Tpl->TemplatePart(PATH_THEME.'tpl/adm/view_noresults.tpl');
endif;